...
def listar_categorias():
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute("SELECT id, titulo AS name FROM categorias ORDER BY titulo")
        return cur.fetchall()
...
def adicionar_categoria(nome):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO categorias (titulo) VALUES (%s) ON CONFLICT (titulo) DO NOTHING",
            (nome,),
        )
        conn.commit()
...
def deletar_categoria(nome):
    with get_conn() as conn, conn.cursor() as cur:
        # ON DELETE SET NULL já limpa id_categoria das músicas automaticamente
        cur.execute("DELETE FROM categorias WHERE titulo = %s", (nome,))
        conn.commit()
...
def _id_categoria_por_nome(cur, nome_categoria):
    if not nome_categoria:
        return None
    cur.execute("SELECT id FROM categorias WHERE titulo = %s", (nome_categoria,))
    row = cur.fetchone()
...
_SELECT_MUSICA = """
    ...
    FROM musicas m
    LEFT JOIN categorias c ON c.id = m.id_categoria
"""
...
def buscar_categoria_por_nome(nome_categoria):
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT id, titulo AS name FROM categorias WHERE LOWER(titulo) = %s",
            (nome_categoria.lower(),),
        )
        return cur.fetchone()
...
def adicionar_musica(titulo, artista, streams, nome_categoria):
    with get_conn() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            """
            INSERT INTO musicas (titulo, artista, streams, id_categoria)
            VALUES (%s, %s, %s, %s)
            """,
